...
class Installer {
	...
	public function do_plugin_install( $activate = true ) {
		...
		if ( ! $this->plugin || ! $this->slug ) {
			$this->log = __( 'Plugin is not registered', 'croco-ik' );
			return false;
		}
		 ...
		 */
		do_action( 'croco-ik/before-plugin-install', $plugin );
		 ...
		 * @param array $plugin Plugin data array.
		 */
		do_action( 'croco-ik/after-plugin-install', $plugin );
		 ...
		 * @param array $plugin Plugin data array.
		 */
		do_action( 'croco-ik/after-plugin-activation', $plugin );
	...
	public function locate_source( $plugin = array() ) {
		...
		switch ( $source ) {
			case 'wordpress':
				...
				if ( is_wp_error( $api ) ) {
					$this->log = __( 'Plugins API error', 'croco-ik' ) . ': ' . $api->get_error_message();
					return false;
				}
			...
			case 'crocoblock':
				...
				if ( $license ) {
					...
					$this->log = __( 'Crocoblock plugins can`t be installed without a license', 'croco-ik' );
					return false;
				}
